...
![](assets/分布式事解决方案/file-20260714190345690.png)
...
1. **发送半事务消息（Half Message）**：生产者首先向 RocketMQ Broker 发送一条半事务消息。Broker 将消息持久化成功后，会向生产者返回 Ack 确认。此时，该消息被标记为“暂不能投递”状态，对下游消费者不可见。
2. **执行本地事务**：生产者收到发送成功的确认后，开始执行本地的业务事务逻辑（如更新数据库订单状态）。
3. **提交二次确认（Commit/Rollback）**：生产者根据本地事务的执行结果，向 Broker 提交二次确认：
    - **Commit（提交）**：Broker 将半事务消息标记为可投递，并正式投递给下游消费者。
    - **Rollback（回滚）**：Broker 将回滚该事务，丢弃半事务消息，下游消费者永远不会收到该消息。
4. **事务回查机制（Transaction Check）**：在断网或生产者应用重启等特殊情况下，如果 Broker 长时间未收到生产者的二次确认结果（或收到 Unknown 状态），会在固定时间后主动向生产者发起消息回查。生产者收到回查后，需检查对应消息的本地事务最终执行结果，并再次向 Broker 提交 Commit 或 Rollback 确认，从而保证最终的数据一致性。
...
## 代码实现
### 步骤1：实现事务监听器
事务监听器需要实现`TransactionListener`接口，包含两个核心方法[-8](https://developer.aliyun.com/ask/681497)[-9](https://developer.baidu.com/article/detail.html?id=3590491)：
```java
...
public class OrderTransactionListener implements RocketMQLocalTransactionListener {
    ...
    public RocketMQLocalTransactionState executeLocalTransaction(Message msg, Object arg) {
        try {
            // 解析消息内容
            String transactionId = msg.getHeaders().getId();
            String messageBody = new String((byte[]) msg.getPayload());
            OrderDTO order = JSON.parseObject(messageBody, OrderDTO.class);
            
            // 执行本地事务：保存订单
            // 注意：这里必须与消息发送在同一个事务上下文中？RocketMQ无法做到，需要自行保证幂等
    ...
    public RocketMQLocalTransactionState checkLocalTransaction(Message msg) {
        ...
        // 根据transactionId查询本地事务执行结果
        boolean isSuccess = orderService.checkTransactionStatus(transactionId);
        
        if (isSuccess) {
            return RocketMQLocalTransactionState.COMMIT;
        } else {
            // 事务进行中或失败，可返回UNKNOWN等待下次回查
...
public class OrderService {
    ...
    public void createOrderWithTransaction(OrderDTO order) {
        ...
        // 构建消息
        MessageBuilder<String> builder = MessageBuilder.withPayload(JSON.toJSONString(order))
                .setHeader("transactionId", transactionId)
                .setHeader(RocketMQHeaders.TRANSACTION_ID, transactionId);
        
        // 发送事务消息
        // sendMessageInTransaction的第三个参数arg会传递给executeLocalTransaction方法
        TransactionSendResult result = rocketMQTemplate.sendMessageInTransaction(
            "TransactionTopic",           // 事务主题
            builder.build(),               // 消息
            order                          // 业务参数
        );
        ...
        if (result.getSendStatus() == SendStatus.SEND_OK) {
            log.info("事务消息发送成功，transactionId: {}, localState: {}", 
                     transactionId, result.getLocalTransactionState());
        } else {
            log.error("事务消息发送失败，transactionId: {}", transactionId);
        }
    }
}
```
...
消费者需要自行保证幂等性，因为消息可能因重试而被重复消费[-6](https://rocketmq.apache.org/zh/docs/featureBehavior/04transactionmessage/?f_link_type=f_linkinlinenote&flow_extra=eyJpbmxpbmVfZGlzcGxheV9wb3NpdGlvbiI6MCwiZG9jX3Bvc2l0aW9uIjowLCJkb2NfaWQiOiI0OTViMWYyMzhjNTQ3NzQwLTYwYTNhMTQ0MDNlMTA1YjUifQ%3D%3D)：
```java
@Component
...
public class InventoryConsumer implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt message) {
        ...
        // 幂等检查：利用数据库唯一键或Redis
        if (idempotentService.isProcessed(transactionId)) {
            log.info("消息已处理过，跳过: {}", transactionId);
            return;
        }
        ...
        try {
            ...
            // 记录处理成功
            idempotentService.recordSuccess(transactionId);
        } catch (Exception e) {
