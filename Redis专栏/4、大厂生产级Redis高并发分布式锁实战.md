...
# 三、面试题，Redis集群情况下，主从切换导致锁丢失怎么解决
**使用RedLock解决**，不推荐使用
![](assets/4、大厂生产级Redis高并发分布式锁实战/file-20260603115518898.png)

RedLock存在的问题，
- Redis集群，当主节点切换的时候依然存在多个线程能加锁成功；
- 如果加很多个节点，加锁性能下降
- 持久化可能丢失数据
...
## 1、加锁的粒度越小越好
## 2、分段锁，
比如商品的库存product_101_stock有一千个库存，拆解为10个key，轮询
product_101_stock放一百个库存，product_102_stock也放一百个库存，依次类推；参考1.7CurrentHashMap的底层实现
## 3、DCL双重检查加锁
![](assets/4、大厂生产级Redis高并发分布式锁实战/file-20260603205923576.png)
## 4、对锁进行拆分，可以通过加分布式读写锁保证并发读写或写写的时候按顺序排好队，读读的时候相当于无锁。（锁的粒度越小越好，范围越小越好）
Redisson 读写锁严格遵循以下互斥规则：
- **读读不互斥**：多个客户端/线程可以同时持有读锁，进行并发读取。
- **读写互斥**：当有线程持有写锁时，其他线程无法获取读锁；反之，有读锁时也无法获取写锁。
- **写写互斥**：同一时间只能有一个线程持有写锁，保证写操作的绝对独占。

### 底层实现原理
Redisson 读写锁的底层核心依赖于 **Redis 的 Lua 脚本原子操作** 和 **Pub/Sub 发布订阅通知机制**：
- **数据结构**：在 Redis 中使用 Hash 结构存储锁信息，包含锁模式（`read`/`write`）、持有锁的客户端标识（`UUID:threadId`）以及锁的重入次数（`count`）。
- **加锁逻辑**：通过 Lua 脚本原子性地判断当前是否存在写锁或读锁。如果允许加读锁，则对读锁计数加 1；如果获取写锁，则需判断是否已有读锁或写锁被占用，若被占用则阻塞等待或返回失败。
- **自动续期（WatchDog）**：支持看门狗机制，在锁未主动释放时自动续期，防止锁因超时被误释放。
